...
function schedule(date: unknown, time: unknown, futureOnly = true) {
  ...
  if (
    ...
    !fixedStartTimes.includes(hour as (typeof fixedStartTimes)[number])
  )
    fail(400, "Choose a valid date and start time.");
  const timestamp = Date.parse(`${day}T${hour}:00-04:00`);
  if (
    !Number.isFinite(timestamp) ||
    new Date(timestamp).toISOString().slice(0, 10) !== day
  )
    fail(400, "Choose a valid calendar date.");
  if (
    futureOnly &&
    (day < buildDateChoices(1)[0].value ||
      day > buildDateChoices(365)[364].value)
  )
    fail(400, "Choose a date from tomorrow through the next 365 days.");
  return { date: day, time: hour, timestamp };
}
function validateDraft(input: unknown) {
  if (!input || typeof input !== "object" || Array.isArray(input))
    fail(400, "Invalid booking request.");
  ...
  if (typeof body.venueType !== "string" || !venues.includes(body.venueType))
    fail(400, "Choose a venue type.");
  const person = body.customer as Record<string, unknown> | undefined;
  if (!person || typeof person !== "object" || Array.isArray(person))
    fail(400, "Enter contact details.");
  if (
    typeof body.weatherFlexible !== "boolean" ||
    typeof person.marketingOptIn !== "boolean"
  )
    fail(400, "Invalid preference selection.");
  const customer = {
    fullName: text(person.fullName, "full name", 120, 2),
    email: text(person.email, "email", 254, 3),
    phone: text(person.phone, "phone", 30, 7),
  ...
  const draft: BookingDraft = {
    ...
    area: text(body.area, "area", 120, 2),
...
export function createApp(config: ServerConfig) {
  ...
  if (config.secureCookies && !origin.startsWith("https://"))
    throw new Error(
      "PUBLIC_ORIGIN must use HTTPS when secure cookies are enabled.",
    );
  ...
  if (
    !(db.prepare("PRAGMA table_info(sessions)").all() as Row[]).some(
      (c) => c.name === "user_id",
    )
  )
    db.exec(
      "ALTER TABLE sessions ADD COLUMN user_id TEXT NOT NULL DEFAULT 'owner'",
    );
  db.exec(
  ...
  app.use("/api", (req, _res, next) => {
    if (!["GET", "HEAD", "OPTIONS"].includes(req.method)) {
      ...
      if (
        !(req.path === "/admin/media" && req.is("multipart/form-data")) &&
        !req.is("application/json")
      )
        return next(new ApiError(415, "Use application/json."));
    }
    next();
  });
  ...
  const authenticate = (req: Request, res: Response, next: NextFunction) => {
    const session = db
      .prepare("SELECT csrf, expires, user_id FROM sessions WHERE token_hash=?")
      .get(digest(token(req))) as Row | undefined;
    ...
    const user =
      session.user_id === "owner"
        ...
        : (db
            ...
            .get(session.user_id) as Row | undefined);
    if (!user?.active)
      return next(new ApiError(401, "Access has been revoked."));
  ...
  app.post("/api/admin/login", limiter(5, 15 * 60_000), async (req, res) => {
    ...
    if (password.length > 256 || username.length > 254)
    ...
    const candidate = (await derive(
      password,
      account ? Buffer.from(account.salt, "hex") : salt,
      64,
    )) as Buffer;
    if (
      !timingSafeEqual(
        candidate,
        account ? Buffer.from(account.password_hash, "hex") : passwordHash,
      ) ||
  ...
  app.post("/api/bookings", limiter(15, 60 * 60_000), (req, res) => {
    ...
    if (!/^[a-f0-9-]{36}$/i.test(key))
      fail(400, "Missing request identifier. Refresh and try again.");
    ...
    try {
      db.prepare(
        ...
        booking.draft.time,
        start,
        start + booking.summary.totalBlockMinutes * 60_000,
        booking.draft.customer.fullName,
        booking.draft.customer.email,
      ...
      const campaign =
        ...
        db
          ...
          .get(req.body.campaignId)
          ? req.body.campaignId
          : "";
      db.prepare("UPDATE bookings SET visit_id=?,campaign_id=? WHERE id=?").run(
        validVisit(req.body.visitId) ? hashVisit(req.body.visitId) : "",
        campaign,
        id,
      );
      db.prepare(
        "INSERT INTO history (booking_id,at,actor,message) VALUES (?,?,?,?)",
      ).run(id, now, "Customer", "Booking request received.");
      db.exec("COMMIT");
    } catch (error) {
      db.exec("ROLLBACK");
      throw error;
  ...
  const detail = (row: Row) => ({
    ...
    history: db
      .prepare(
        "SELECT at,actor,message FROM history WHERE booking_id=? ORDER BY id DESC",
      )
      .all(row.id),
  });
  app.get("/api/admin/bookings", (req, res) => {
    const q =
      typeof req.query.q === "string"
        ? req.query.q.slice(0, 120).toLowerCase()
        : "";
    const status = typeof req.query.status === "string" ? req.query.status : "";
    const date = typeof req.query.date === "string" ? req.query.date : "";
    if (status && !statuses.includes(status))
      fail(400, "Invalid status filter.");
    if (date && !/^\d{4}-\d{2}-\d{2}$/.test(date))
      fail(400, "Invalid date filter.");
    const page = Math.min(
      100000,
      Math.max(1, parseInt(String(req.query.page), 10) || 1),
    );
    const conditions =
      "(?='' OR status=?) AND (?='' OR date=?) AND (?='' OR instr(lower(name || ' ' || email || ' ' || reference),?)>0)";
    const params = [status, status, date, date, q, q];
    const total = (
      db
        .prepare(`SELECT COUNT(*) AS n FROM bookings WHERE ${conditions}`)
        .get(...params) as Row
    ).n;
    const rows = db
      .prepare(
        `SELECT * FROM bookings WHERE ${conditions} ORDER BY date,time,created_at LIMIT 25 OFFSET ?`,
      )
      .all(...params, (page - 1) * 25) as Row[];
    ...
    res.json({
      ...
      bookings: rows.map((row) => {
        ...
        return {
          ...
          players: payload.draft.players,
          area: payload.draft.area,
        };
      }),
    });
  });
  function permittedDetail(row: Row, res: Response) {
    const value = detail(row);
    if (res.locals.user.role !== "admin" && !res.locals.user.finance) {
      delete value.summary.totalPrice;
      delete value.summary.currency;
      delete value.mission.price;
      delete value.mission.currency;
    }
    return value;
  }
  ...
  installKpis(app, db, config.publicOrigin, fail);
  app.get("/api/admin/bookings/:id", (req, res) =>
    res.json(permittedDetail(getBooking(String(req.params.id)), res)),
  );
  app.patch("/api/admin/bookings/:id", (req, res) => {
    const id = String(req.params.id);
    if (!req.body || typeof req.body !== "object") fail(400, "Invalid update.");
    const { status, version } = req.body;
    if (!statuses.includes(status) || !Number.isInteger(version))
      fail(400, "Choose a valid status.");
    ...
    try {
      ...
      if (row.version !== version)
        fail(
          409,
          "This booking changed in another session. Reload it before saving.",
        );
      const payload = JSON.parse(row.payload);
      const changedSchedule =
        req.body.date !== row.date || req.body.time !== row.time;
      const slot = schedule(req.body.date, req.body.time, changedSchedule);
      if (status === "confirmed" && slot.timestamp < Date.now())
        fail(400, "A past event cannot be confirmed. Reschedule it first.");
      if (status === "completed" && slot.timestamp > Date.now())
        fail(400, "A future event cannot be marked completed.");
      const end = slot.timestamp + payload.summary.totalBlockMinutes * 60_000;
      if (status === "confirmed") {
        const conflict = db
          .prepare(
            "SELECT reference FROM bookings WHERE id<>? AND status='confirmed' AND start_ms < ? AND end_ms > ? LIMIT 1",
          )
          .get(id, end, slot.timestamp) as Row | undefined;
        if (conflict)
          fail(
            409,
            `Overlaps confirmed booking ${conflict.reference}, including its operating buffer. Choose another time.`,
          );
      }
      payload.draft.date = slot.date;
      payload.draft.time = slot.time;
      const now = new Date().toISOString();
      db.prepare(
        "UPDATE bookings SET status=?,date=?,time=?,start_ms=?,end_ms=?,payload=?,internal_notes=?,updated_at=?,version=version+1 WHERE id=?",
      ).run(
        status,
        slot.date,
        slot.time,
      ...
      const changes =
        [
          ...
          changedSchedule
            ? `Schedule: ${row.date} ${row.time} → ${slot.date} ${slot.time} (Saint Lucia).`
            : "",
          notes !== row.internal_notes ? "Internal notes updated." : "",
        ]
          .filter(Boolean)
          .join(" ") || "Booking reviewed.";
      db.prepare(
        "INSERT INTO history (booking_id,at,actor,message) VALUES (?,?,?,?)",
      ).run(id, now, res.locals.user.name, changes);
      db.exec("COMMIT");
      res.json(permittedDetail(getBooking(id), res));
    } catch (error) {
      db.exec("ROLLBACK");
      throw error;
    }
